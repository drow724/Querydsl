...
//		Member fetchFirst = queryFactory.selectFrom(member).fetchFirst();
	...
	public void join() {
		List<Member> result = queryFactory.selectFrom(member)
				...
				.leftJoin(member.team, team).where(team.name.eq("teamA")).fetch();
	 ...
	 * m LEFT JOIN m.team t on t.name = 'teamA' SQL: SELECT m.*, t.* FROM Member m
	...
	/**
	 * 2. 연관관계 없는 엔티티 외부 조인 예) 회원의 이름과 팀의 이름이 같은 대상 외부 조인 JPQL: SELECT m, t FROM
	 * Member m LEFT JOIN Team t on m.username = t.name SQL: SELECT m.*, t.* FROM
	...
	@Test
	public void dynamicQuery_BooleandBuilder() throws Exception {
